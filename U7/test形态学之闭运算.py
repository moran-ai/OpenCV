...
while True:
    ...
    # 开运算操作
    # 第一个参数是二值化图片
    rect = cv.morphologyEx(thresh,cv.MORPH_CLOSE,rectkernel)
    cross = cv.morphologyEx(thresh,cv.MORPH_CLOSE,crosskernel)
    ellip = cv.morphologyEx(thresh,cv.MORPH_CLOSE,ellipkernel)
