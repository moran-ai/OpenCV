...
def nothing(x):
    pass
cv.createTrackbar('ks','TOPHAT',2,25,nothing) # 2为滑动条初始值，25为最大值
...
while True:
    ...
    # 开运算操作
    # 第一个参数是二值化图片
    rect = cv.morphologyEx(thresh, cv.MORPH_TOPHAT, rectkernel)
    cross = cv.morphologyEx(thresh,cv.MORPH_TOPHAT,crosskernel)
    ellip = cv.morphologyEx(thresh,cv.MORPH_TOPHAT,ellipkernel)
